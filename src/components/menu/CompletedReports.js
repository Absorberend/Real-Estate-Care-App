...
export default function CompletedReports({data, onCategoryDisplay}) {
  ...
  if (data) {
    viewReport = data.filter(report => report.completed === "true").sort((a,b) => new Date(b.date) - new Date(a.date)).map(report => {
      ...
      >
        ...
        <span>
          <img src={chrevronDown} alt="expand report" className="completed__reports__button__icon__hidden"/>
        </span>
        <img src={filteredReport[0]?.id === report.id ? chrevronUp : chrevronDown} alt="expand report" className="completed__reports__button__icon" />
      </button>
      {filteredReport[0]?.id === report.id && <div key={`filtered${report.id}`} className="completed__reports__overview__container">
        ...
        {report?.modificationEncountered && <div className="completed__reports__overview__expanded__reports">
          <span>Gedocumenteerde modificaties:</span>
          <img src={report.modificationEncountered} alt="gedocumenteerde modificatie" />
        </div>}
        {report?.installationType && <div className="completed__reports__overview__expanded__reports">
          <span>Soort installatie:</span>
          <span>{report.installationType}</span> 
        </div>}
        {report?.maintenanceType && <div className="completed__reports__overview__expanded__reports">
          <span>Soort onderhoud:</span>
          <span>{report.maintenanceType}</span> 
        </div>}
        {report?.newDamages && <div className="completed__reports__overview__expanded__reports">
          <span>Nieuwe schade:</span>
          <span>{report.newDamages}</span> 
        </div>}
        {report?.carriedOutBy && <div className="completed__reports__overview__expanded__reports">
          <span>Uitgevoerd door:</span>
          <span>{report.carriedOutBy}</span> 
        </div>}
        {report?.reportedMalfunction && <div className="completed__reports__overview__expanded__reports">
          <span>Gemelde storingen:</span>
          <span>{report.reportedMalfunction}</span> 
        </div>}
        {report?.acuteActionRequired && <div className="completed__reports__overview__expanded__reports">
          <span>Acute actie vereist:</span>
          <span>{report.acuteActionRequired}</span> 
        </div>}
        {report?.damageType && <div className="completed__reports__overview__expanded__reports">
          <span>Soort schade:</span>
          <span>{report.damageType}</span> 
        </div>}
        {report?.descriptionModification && <div className="completed__reports__overview__expanded__reports">
        ...
        {report?.action && <div className="completed__reports__overview__expanded__reports">
          <span>Te ondernemen actie:</span>
          <span>{report.action}</span> 
        </div>}
        {report?.approved && <div className="completed__reports__overview__expanded__reports">
          <span>Goedgekeurd:</span>
          <span>{report.approved}</span> 
        </div>}
        {report?.description && <div className="completed__reports__overview__expanded__reports">
        ...
        <div className="overview__container__delete__modal">
          ...
          <div className="overview__container__delete__modal__buttons__wrapper">
            <button onClick={() => {
              ...
              >Ja</button>
            <button className="overview__container__delete__modal__button" onClick={() => handleDeleteModalClick()}>Nee</button>
          </div>
  ...
  return (
    <>
      {locationURL.pathname === "/CompletedReports" && <div className="completed__reports__back__button">
        <BackButton />
      </div>}
